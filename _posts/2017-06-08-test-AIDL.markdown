...
date:   2017-6-8 08:50:24 +0800
categories: 转载：http://www.jianshu.com/p/3303752057be
tag: 教程
---
...
在这个例子中我们编写一个名为RemoteService的Service类，并在onBind()方法中返回上述第二步中实现的接口,这样就把接口传给了客户端供其调用:
...
在onServiceConnected()回调中，我们使用IRemoteService.Stub.asInterface(Binder)方法返回我们的接口的引用。接着客户端就可以通过它来对服务端发送请求了。
